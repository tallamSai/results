...
export default function DepartmentHeadReviewstheevaluation({ workflowInstanceId = null, previousStageData = null, onSubmit, loading, initialData = {}, disabled = false }) {
  const [formData, setFormData] = useState(initialData);
  
  // Update formData when initialData changes (important for read-only display)
  useEffect(() => {
    if (initialData && Object.keys(initialData).length > 0) {
      console.log(`📝 Form (DepartmentHeadReviewstheevaluation): Loading initialData:`, initialData);
...
const handleSubmit = (e) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      />
      <EmployeeHeader 
        title="Department Head - Reviews the evaluation"
        subtitle="Department Head Process"
      ...
      <div className="max-w-6xl mx-auto px-6 py-3">
        <div className="flex items-center text-sm text-gray-600">
          ...
          <span className="text-gray-900 font-medium">Department Head - Reviews the evaluation</span>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-md border border-gray-200">
          <div className="px-8 py-6">
            ...
            >
              <input
                type="radio"
                name="question_27dce6c5"
                value="Approve"
                checked={formData['question_27dce6c5'] === 'Approve'}
                onChange={e => handleInputChange('question_27dce6c5', e.target.value)}
            ...
            >
              <input
                ...
                name="question_27dce6c5"
                value="Disapprove"
                checked={formData['question_27dce6c5'] === 'Disapprove'}
                onChange={e => handleInputChange('question_27dce6c5', e.target.value)}
                className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
              />
              <span className="text-sm text-gray-700">Disapprove</span>
            </label>
                  </div>
                </div>
              </div>
                <div className="flex flex-col gap-2 mb-6">
                  ...
                  <div className="bg-white border border-gray-300 rounded-md">
                    <textarea
                      value={formData['question_ce5479f4'] || ''}
                      onChange={e => handleInputChange('question_ce5479f4', e.target.value)}
                      placeholder="Enter any comments or feedback regarding the evaluation"
                      rows="4"
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-gray-200">
                <button
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    console.log('Form rejected:', { formData, participant: 'Department Head', action: 'reject' });
                ...
                <button
                  ...
                  onClick={(e) => {
                    console.log('Form approved:', { formData, participant: 'Department Head', action: 'approve' });
                  }}
                >
                  Approve
                </button>
